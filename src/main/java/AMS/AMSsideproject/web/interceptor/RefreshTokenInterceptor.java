...
public class RefreshTokenInterceptor implements HandlerInterceptor {
    ...
    public boolean preHandle(HttpServletRequest request, HttpServletResponse response, Object handler) throws Exception {
        ...
        if(!StringUtils.hasText(refreshToken)) //헤더에 리프레시 토큰이 없는 경우
            errorResult = new ErrorResult("리프레시 토큰이 없거나 잘못된 값입니다.","BAD","400");
       ...
       //헤더에 리프레시 토큰이 있는 경우
        try{
            ...
            errorResult = new ErrorResult("리프레시토큰이 만료되었습니다. 다시 로그인 해주시기 바랍니다.", "BAD", "400");
        }catch (Exception e) { //토큰 정보가 정상적이지 않은 경우
            errorResult = new ErrorResult("리프레시 토큰이 없거나 잘못된 값입니다. 다시 로그인을 해주시기 바랍니다.","BAD","400");
        }
