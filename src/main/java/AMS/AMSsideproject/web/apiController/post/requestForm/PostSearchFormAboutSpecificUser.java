...
@NoArgsConstructor
@AllArgsConstructor
public class PostSearchFormAboutSpecificUser {

    @ApiModelProperty(example = "[]", notes = "없을시 빈배열로 주시면 됩니다.")
    @NotNull(message = "필수 입렵값 입니다.")
    private List<String> tags; //tags

    @ApiModelProperty(example = "SEE", notes = "SEE/ALONE, 없을시 공백으로 채워주시면 됩니다.")
    @NotNull(message = "필수 입렵값 입니다.")
    @Pattern(regexp = "SEE|ALONE|^$", message = "SEE,ALONE,빈문자열 중 하나입니다.")
