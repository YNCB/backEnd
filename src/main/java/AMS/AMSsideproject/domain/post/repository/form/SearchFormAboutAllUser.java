...
public class SearchFormAboutAllUser {
    ...
    @ApiModelProperty(example = "redata(최신순)", notes = "정렬기준 -> redate(최신순), likeNum(좋아요순), replyNum(댓글많은순)")
    private String orderKey; // 정렬 기준 -> redate(최신순), likeNum(좋아요순), replyNum(댓글많은순)

    @ApiModelProperty(example = "1", notes = "무한스크롤 방식을 위한것, 마지막 게시물의 게시물 고유 id(post_id)입니다. " +
            "redate(최신순),likeNum(좋아요순),replyNum(댓글많은순) 모두 해당합니다. ")
